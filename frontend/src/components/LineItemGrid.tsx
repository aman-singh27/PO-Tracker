...
export function LineItemGrid({ value, onChange, gstRate = "0" }: Props) {
  ...
  const move = (event: React.KeyboardEvent, row: number, col: number) => {
    if (event.key === "Enter" && col === 4) { event.preventDefault(); onChange([...rows, blank(rows.length + 1, gstRate)]); setTimeout(() => inputs.current[(rows.length) * 5]?.focus()); return }
  ...
  return <div className="grid-wrap" aria-label="Line item grid"><table className="data-table line-grid"><thead><tr><th>Description</th><th>Type</th><th className="num">Qty</th><th>Unit</th><th className="num">Rate</th><th className="num">Amount</th></tr></thead><tbody>{rows.map((line, row) => <tr key={row}><td><input ref={ref(row * 5)} aria-label={`Line ${row + 1} description`} value={line.description} onPaste={e => paste(e, row, 0)} onKeyDown={e => move(e, row, 0)} onChange={e => change(row, "description", e.target.value)} /></td><td><select ref={ref(row * 5 + 1)} aria-label={`Line ${row + 1} type`} value={line.item_type} onKeyDown={e => move(e, row, 1)} onChange={e => change(row, "item_type", e.target.value)}><option value="material">Material</option><option value="service">Service</option></select></td><td><input ref={ref(row * 5 + 2)} inputMode="decimal" className="num" aria-label={`Line ${row + 1} quantity`} value={line.qty_ordered} onKeyDown={e => move(e, row, 2)} onChange={e => change(row, "qty_ordered", e.target.value)} /></td><td><input ref={ref(row * 5 + 3)} aria-label={`Line ${row + 1} unit`} value={line.unit} onKeyDown={e => move(e, row, 3)} onChange={e => change(row, "unit", e.target.value)} /></td><td><input ref={ref(row * 5 + 4)} inputMode="decimal" className="num" aria-label={`Line ${row + 1} rate`} value={line.rate} onKeyDown={e => move(e, row, 4)} onChange={e => change(row, "rate", e.target.value)} /></td><td className="num">{lineAmount(line.qty_ordered, line.rate)}</td></tr>)}</tbody></table><p className="helper">Tab moves between cells · Enter on the last cell adds a row · Ctrl+D copies from above · Paste rows from Excel into a description cell.</p></div>
}
