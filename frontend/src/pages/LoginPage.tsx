...
export function LoginPage() {
  ...
  async function submit(event: React.FormEvent) { event.preventDefault(); setBusy(true); setError(""); try { const { data } = await authApi.login(email, password); setUser(data); navigate((location.state as { from?: string } | null)?.from || "/") } catch { setError("We could not sign you in. Check your email and password.") } finally { setBusy(false) } }
